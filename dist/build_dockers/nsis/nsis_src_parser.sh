...
for DESTINATION in "${DESTINATION_REPOS[@]}"; do
  ...
  if [ -n "$MANTICORE_PACKAGE_NAME" ] && [ -n "$BUDDY_PACKAGE_NAME" ] && [ -n "$EXECUTOR_VERSION" ] && [ -n "$MCL_PACKAGE_NAME" ]; then
...
if [ -z "$MANTICORE_PACKAGE_NAME" ] || [ -z "$BUDDY_PACKAGE_NAME" ] || [ -z "$EXECUTOR_VERSION" ] || [ -z "$MCL_PACKAGE_NAME" ]; then
